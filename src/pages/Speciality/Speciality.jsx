...
export default function Speciality() {
  ...
  useEffect(() => {
    for (let i = 0; i < specailities.length; i++) {
      if (speciality == specailities[i].speciality_name) {
  ...
  return (
    <div
      ...
      id="Speciality"
    >
      <section className="w-full bg-[#700619] flex flex-col justify-center items-center py-10">
        <div className="flex items-center flex-wrap justify-between mt-10 md:flex-col w-11/12 gap-6">
          {data.map((item, index) => (
            <Link
              to={`/Speciality/${item.name}`}
              className={`Speciality-card ${
                speciality === item.name ? "selected-card" : ""
              }`}
            >
      ...
      <section className="flex flex-col gap-6 w-11/12">
        <p className="Speciality-hospitals-title my-6">
          {selectedSpeciality && selectedSpeciality.hospitals.length} Healthcare
          Partners in {selectedSpeciality && selectedSpeciality.speciality_name}
        </p>
        <div className="flex w-full md:flex-col justify-between items-center flex-wrap gap-y-16">
          {selectedSpeciality &&
            selectedSpeciality.hospitals.map((hospital, index) => (
              <div className="Speciality-hospitals-card" key={index}>
                <img
                ...
                <div className="flex flex-col justify-between md:gap-4 md:py-1">
                  <p className="Speciality-hospitals-card-title">
                    {hospital.name}
                  </p>
                  <p className="flex items-center Speciality-hospitals-card-text">
                    <svg
                  ...
                  <p className="Speciality-hospitals-card-text">
                    Facilty Type: {hospital.facility_type}
                  </p>
                  <p className="Speciality-hospitals-card-text flex flex-wrap">
                    <p>Service Offered:</p>
                    {hospital.services.map((item, index) => (
                      <div className="Speciality-hospitals-card-chip">
                        {item}
                      </div>
                    ))}
                  </p>
                  <Link to={`/hospital/${hospital.link}`}>
                    <button className="Speciality-hospitals-card-button">
                      View Profile
                    </button>
                  </Link>
                </div>
